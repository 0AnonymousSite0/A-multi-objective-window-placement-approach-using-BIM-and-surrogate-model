...
def WallGeometryInformationTransformation(ifc_file_path, idf_file_path):
    ...
    for wall in walls:
        ...
        shape = wall.Representation
        length, width, height = get_wall_dimensions(shape) if shape else (None, None, None)
        print(f'Wall ID: {global_id}, Name: {name}, Location: {coordinates}, Dimensions: {length}x{width}x{height}')
...
WallGeometryInformationTransformation(ifc_file_path, idf_file_path)
