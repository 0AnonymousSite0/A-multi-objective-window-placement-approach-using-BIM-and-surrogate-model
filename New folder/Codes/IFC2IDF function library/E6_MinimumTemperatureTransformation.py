...
def MinimumTemperatureTransformation(ifc_file_path, idf_file_path):
    ...
    def replace_value_in_idf_near_marker(idf_file_path, marker, new_value):
        ...
        if marker_index is not None and marker_index < len(lines):
            ...
            if len(line_parts) > 0:
                line_parts[0] = f"    {new_value}"  # Ensure formatting matches IDF standards
                lines[marker_index] = ','.join(line_parts)

            with open(idf_file_path, 'w') as file:
                file.writelines(lines)
            print(f"Successfully replaced the value with {new_value} at the marker '{marker}'.")
        else:
            print("Marker not found or insufficient lines near marker.")

    property_name = "MinimumTemperature"
    property_value = extract_property_value_from_ifc(ifc_file_path, property_name)

    if property_value is not None:
        # Replace the value at the line with the specific marker
        marker = "!- Minimum Cooling Supply Air Temperature {C}"
        replace_value_in_idf_near_marker(idf_file_path, marker, property_value)
    else:
        print("Property not found in IFC file.")

ifc_file_path = r'path of ifc file'
idf_file_path = r'path of idf file'
MinimumTemperatureTransformation(ifc_file_path, idf_file_path)
